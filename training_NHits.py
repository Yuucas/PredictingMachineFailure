...
if __name__ == "__main__":
    ...
    best_hyperparams = {
        'input_chunk_length': 118,
        'output_chunk_length': 30,
        'num_stacks': 4,
        'num_blocks': 2,
        'num_layers': 3,
        'layer_widths': 141, # N-HiTS might prefer powers of 2 or standard sizes, but we'll use what Optuna found
        'dropout': 0.11798003756140936,
        'optimizer_kwargs': {'lr': 0.0001978520671385454},
    ...
    wandb_run_name = f"NHITS_Final_in{best_hyperparams['input_chunk_length']}_lr{best_hyperparams['optimizer_kwargs']['lr']:.0e}"
    ...
    final_model_params = {
        ...
        'model_name': "NHits_Final_Trained_Model", # For local checkpoints
    ...
    final_model = NHiTSModel(**final_model_params)

    print("\n--- Training Final N-HiTS Model (Tracked by W&B) ---")
    ...
    if len(train_target_ts) < final_model_params['input_chunk_length'] + final_model_params['output_chunk_length'] or \
       (val_target_ts is not None and len(val_target_ts) < final_model_params['output_chunk_length']):
        ...
        print("\n--- Final N-HiTS Model Training Complete ---")

        final_model_save_path = os.path.join(final_model_params['work_dir'], final_model_params['model_name'], "final_best_model.pt")
        ...
        model_artifact = wandb.Artifact(
            ...
            type="model",
            description=f"Final trained N-HiTS model after hyperparameter tuning. Best val_loss.",
    ...
    if len(test_target_ts) > 0:
        ...
        try:
            ...
            probabilistic_predictions_nhits = model_to_predict_with.predict(
            ...
            predictions_nhits = probabilistic_predictions_nhits.quantile_timeseries(quantile=0.5)

            # Make sure it uses `test_target_ts` for actuals and `predictions_nhits` for predicted.
            ...
            if min_len > 0:
                ...
                ax_cm.set_title('Final N-HiTS Model - Confusion Matrix')
                ...
                x_min_display, x_max_display = 9000, 9050 # Adjusted range for example
